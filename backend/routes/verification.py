...
async def verify_student(request: VerifyStudentRequest, db: Session = Depends(get_db)):
    ...
    career = db.query(Career).filter(Career.name.ilike(f"%{career_name_normalized}%")).first()
    ...
    # 2. Normalizar el nombre de entrada (ej: "Deyvis Aguilar" -> "DEYVIS AGUILAR")
    ...
    # Creamos filtros dinámicos: para cada palabra del input, buscamos si está dentro de full_name
    filters = []
    for word in words:
        filters.append(UniversityStudent.full_name.like(f"%{word}%"))

    query = db.query(UniversityStudent).filter(UniversityStudent.career_id == career.id)
